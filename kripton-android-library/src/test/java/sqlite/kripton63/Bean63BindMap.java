...
public class Bean63BindMap extends AbstractMapper<Bean63> {
   ...
   */
  @Override
  public void serializeOnXml(Bean63 object, XMLSerializer xmlSerializer, EventType currentEventType)
      throws Exception {
   ...
   */
  @Override
  public Bean63 parseOnXml(XMLParser xmlParser, EventType currentEventType) throws Exception {
    Bean63 instance = new Bean63();
                  ...
                  // property valueMapEnumByte (mapped on "valueMapEnumByte")
                   {
                    ...
                    xmlParser.nextTag();
                    collection.put(key, value);
                    while (xmlParser.nextTag() != EventType.END_TAG && xmlParser.getName().toString().equals("valueMapEnumByte")) {
                      xmlParser.nextTag();
                      key=EnumType.valueOf(StringEscapeUtils.unescapeXml(xmlParser.getElementText()));
                      xmlParser.nextTag();
                      if (xmlParser.isEmptyElement()) {
                  ...
                  // property valueMapStringByte (mapped on "valueMapStringByte")
                   {
                    ...
                    xmlParser.nextTag();
                    collection.put(key, value);
                    while (xmlParser.nextTag() != EventType.END_TAG && xmlParser.getName().toString().equals("valueMapStringByte")) {
                      xmlParser.nextTag();
                      key=StringEscapeUtils.unescapeXml(xmlParser.getElementText());
                      xmlParser.nextTag();
                      if (xmlParser.isEmptyElement()) {
