            ...
            switch(currentTag) {
                case "beanList":
                  // property beanList (mapped on "beanList")
                   {
                    ArrayList<Bean02> collection=CollectionUtils.merge(new ArrayList<>(), instance.getBeanList());
                    Bean02 item;
                    // add first element
                    item=null;
                    ...
                    while (xmlParser.nextTag() != EventType.END_TAG && xmlParser.getName().toString().equals("beanList")) {
                      if (XmlAttributeUtils.isEmptyTag(xmlParser)) {
                        item=null;
                        xmlParser.nextTag();
                      } else {
                ...
                case "lista":
                  // property lista (mapped on "lista")
                   {
                    ArrayList<Bean02> collection=CollectionUtils.merge(new ArrayList<>(), instance.getLista());
                    Bean02 item;
                    // add first element
                    item=null;
                    if (XmlAttributeUtils.isEmptyTag(xmlParser)) {
                    ...
                    while (xmlParser.nextTag() != EventType.END_TAG && xmlParser.getName().toString().equals("lista")) {
                      if (XmlAttributeUtils.isEmptyTag(xmlParser)) {
                        item=null;
                        xmlParser.nextTag();
                      } else {
