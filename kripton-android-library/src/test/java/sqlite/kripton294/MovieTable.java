...
public class MovieTable implements SQLiteTable {
   ...
   * </p>
   *
   * <pre>CREATE TABLE movie (mid INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, director_id INTEGER, title TEXT, FOREIGN KEY(director_id) REFERENCES director(mid) ON DELETE CASCADE); CREATE INDEX idx_movie_0 on movie (title); CREATE INDEX idx_movie_1 on movie (director_id);</pre>
   */
  public static final String CREATE_TABLE_SQL = "CREATE TABLE movie (mid INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, director_id INTEGER, title TEXT, FOREIGN KEY(director_id) REFERENCES director(mid) ON DELETE CASCADE); CREATE INDEX idx_movie_0 on movie (title); CREATE INDEX idx_movie_1 on movie (director_id);";
