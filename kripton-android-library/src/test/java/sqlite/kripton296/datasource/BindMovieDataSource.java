...
import sqlite.kripton296.model.DirectorTable;
import sqlite.kripton296.model.MovieTable;

/**
 * <p>
 * Implementation of the MovieDataSource datasource.
 * This class expose database interface through Dao attribute.
 * </p>
 *
 * @see MovieDataSource
 * @see BindMovieDaoFactory
 * @see DirectorDao
 * @see DirectorDaoImpl
 * @see Director
 * @see MovieDao
 ...
 * @see Movie
 */
public class BindMovieDataSource extends AbstractDataSource implements BindMovieDaoFactory, MovieDataSource {
  /**
   * <p>datasource singleton</p>
   */
  static volatile BindMovieDataSource instance;
  ...
  /**
   * Unique identifier for Dao DirectorDao
   */
  public static final int DIRECTOR_DAO_UID = 0;

  /**
   * Unique identifier for Dao MovieDao
   */
  public static final int MOVIE_DAO_UID = 1;

  /**
   * List of tables compose datasource
   */
  static final SQLiteTable[] TABLES = {new DirectorTable(), new MovieTable()};

  /**
   * <p>dao instance</p>
   */
  protected DirectorDaoImpl directorDao = new DirectorDaoImpl(this);

  /**
   * <p>dao instance</p>
   */
  protected MovieDaoImpl movieDao = new MovieDaoImpl(this);
  ...
  protected BindMovieDataSource(DataSourceOptions options) {
    super("movie.db", 1, options);
  }
  ...
  public boolean clearDb() {
    return BindMovieDataSource.this.execute(daoFactory -> {
      return MovieDataSource.clearDb(daoFactory.getMovieDao(), daoFactory.getDirectorDao());
    });
  }
   ...
   */
  @Override
  public Future<Boolean> clearDbAsync() {
    return BindMovieDataSource.this.executeAsync(daoFactory -> { 
      if (daoFactory.clearDb()==true) { return TransactionResult.COMMIT; } else { return TransactionResult.ROLLBACK; }
    });
  }
   ...
   */
  public static BindMovieDataSource getInstance() {
   ...
   */
  public static BindMovieDataSource open() {
    BindMovieDataSource instance=getInstance();
    instance.openWritableDatabase();
    return instance;
  }
   ...
   * @return opened dataSource instance.
   */
  public static BindMovieDataSource openReadOnly() {
    BindMovieDataSource instance=getInstance();
  ...
  protected void onCreate(SupportSQLiteDatabase database) {
    ...
    // log section create BEGIN
    if (this.logEnabled) {
      Logger.info("DDL: %s",DirectorTable.CREATE_TABLE_SQL);
    }
    // log section create END
    database.execSQL(DirectorTable.CREATE_TABLE_SQL);
    // log section create BEGIN
    if (this.logEnabled) {
      Logger.info("DDL: %s",MovieTable.CREATE_TABLE_SQL);
    }
    // log section create END
    database.execSQL(MovieTable.CREATE_TABLE_SQL);
  ...
  public void clearCompiledStatements() {
    DirectorDaoImpl.clearCompiledStatements();
    MovieDaoImpl.clearCompiledStatements();
  }
   ...
   */
  public interface Transaction extends AbstractDataSource.AbstractExecutable<BindMovieDaoFactory> {
    /**
     ...
     * @throws Throwable
     */
    TransactionResult onExecute(@NonNull BindMovieDaoFactory daoFactory);
  }
  ...
  public interface Batch<T> {
     ...
     * @throws Throwable
     */
    T onExecute(@NonNull BindMovieDaoFactory daoFactory);
  }

  class DataSourceSingleThread implements BindMovieDaoFactory {
    private SQLContextInSessionImpl _context;

    protected DirectorDaoImpl _directorDao;

    protected MovieDaoImpl _movieDao;

    DataSourceSingleThread() {
      _context=new SQLContextInSessionImpl(BindMovieDataSource.this);
    }
    ...
    public DirectorDaoImpl getDirectorDao() {
      if (_directorDao==null) {
        _directorDao=new DirectorDaoImpl(this);
      }
      return _directorDao;
    }
    ...
    public MovieDaoImpl getMovieDao() {
      if (_movieDao==null) {
        _movieDao=new MovieDaoImpl(this);
      }
      return _movieDao;
    }
    ...
    protected void onSessionClosed() {
      ...
      if (_directorDao!=null && daosWithEvents.contains(DIRECTOR_DAO_UID)) {
        _directorDao.invalidateLiveData();
      }
      if (_movieDao!=null && daosWithEvents.contains(MOVIE_DAO_UID)) {
        _movieDao.invalidateLiveData();
      }
    }
