...
public class BeanDaoImpl extends Dao implements BeanDao {
  ...
  public BeanBean selectOne() {
    ...
    try (Cursor _cursor = database().rawQuery(_sql, _sqlArgs)) {
      ...
      if (_cursor.moveToFirst()) {
        ...
        if (!_cursor.isNull(index1)) { resultBean.setValue(BeanBeanTable.parseValue(_cursor.getBlob(index1))); }
        if (!_cursor.isNull(index2)) { resultBean.setValue2(BeanBeanTable.parseValue2(_cursor.getBlob(index2))); }
  ...
  public long insert(BeanBean bean) {
    ...
    _contentValues.put("value", BeanBeanTable.serializeValue(bean.getValue()));
    _contentValues.put("value2", BeanBeanTable.serializeValue2(bean.getValue2()));
