...
public class BindDummy03DataSource extends AbstractDataSource implements BindDummy03DaoFactory, Dummy03DataSource {
   ...
   */
  public static BindDummy03DataSource getInstance() {
      ...
      synchronized(mutex) {
        ...
        if (result==null) {
          DataSourceOptions options=DataSourceOptions.builder()
          	...
          	.log(true)
          	.build();
          instance=result=new BindDummy03DataSource(options);
          try {
  ...
  public static BindDummy03DataSource open() {
    BindDummy03DataSource instance=getInstance();
    instance.openWritableDatabase();
    return instance;
  }
  ...
  public static BindDummy03DataSource build(DataSourceOptions options) {
    if (options.forceBuild && instance!=null) {
      Logger.info("Datasource BindDummy03DataSource is forced to be (re)builded");
      instance=null;
    }
    BindDummy03DataSource result=instance;
    if (result==null) {
      synchronized(mutex) {
        ...
        if (result==null) {
          ...
          throw new KriptonRuntimeException("Datasource BindDummy03DataSource is already builded");
        }
      }
    } else {
      throw new KriptonRuntimeException("Datasource BindDummy03DataSource is already builded");
    }
    Logger.info("Datasource BindDummy03DataSource is created");
    return result;
  }
  ...
  public interface Transaction extends AbstractDataSource.AbstractExecutable<BindDummy03DaoFactory> {
     ...
     * @throws Throwable
     */
    TransactionResult onExecute(@NonNull BindDummy03DaoFactory daoFactory);
  }
  ...
  public interface Batch<T> {
     ...
     * @throws Throwable
     */
    T onExecute(@NonNull BindDummy03DaoFactory daoFactory);
  }
