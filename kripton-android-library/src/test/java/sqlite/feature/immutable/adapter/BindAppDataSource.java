...
public class BindAppDataSource extends AbstractDataSource implements BindAppDaoFactory, AppDataSource {
  ...
  public <T> Observable<T> execute(final ObservableTransaction<T> transaction) {
    ObservableOnSubscribe<T> emitter=new ObservableOnSubscribe<T>() {
      ...
      public void subscribe(ObservableEmitter<T> emitter) {
        ...
        Pair<Boolean, SQLiteDatabase> _status=BindAppDataSource.this.openDatabaseThreadSafeMode(true);
  ...
  class DataSourceSingleThread implements BindAppDaoFactory {
    ...
    protected void onSessionClosed() {
      ...
      if (_personDAO!=null && daosWithEvents.contains(PERSON_D_A_O_UID)) {
        _personDAO.invalidateLiveData();
      }
    }
