...
/**
 * <p>
 * Implementation of the AppDataSource datasource. This class expose database
 * interface through Dao attribute.
 * </p>
 *
 * @see AppDataSource
...
public class BindAppDataSource extends AbstractDataSource implements BindAppDaoFactory, AppDataSource {
		...
		ObservableOnSubscribe<T> emitter = new ObservableOnSubscribe<T>() {
			...
			public void subscribe(ObservableEmitter<T> emitter) {
				...
				try {
					...
					if (transaction != null
							&& TransactionResult.COMMIT == transaction.onExecute(currentDaoFactory, emitter)) {
	...
	public <T> Observable<T> executeBatch(final ObservableBatch<T> batch, final boolean writeMode) {
		ObservableOnSubscribe<T> emitter = new ObservableOnSubscribe<T>() {
			...
			public void subscribe(ObservableEmitter<T> emitter) {
				// lock the database
				beginLock();
				boolean needToOpened = writeMode ? !BindAppDataSource.this.isOpenInWriteMode()
						: !BindAppDataSource.this.isOpen();
				// unlock the database
				endLock();
				if (needToOpened) {
					if (writeMode) {
				...
				try {
					if (batch != null) {
						batch.onExecute(currentDaoFactory, emitter);
					}
					emitter.onComplete();
				} catch (Throwable e) {
					Logger.error(e.getMessage());
	 ...
	 * can be executed. The database will be open in write mode. This method
	 * uses default error listener to intercept errors.
	 * </p>
	 *
	 * @param transaction
	 ...
	 * @return <code>true</code> if transaction successful finished
	 */
	public boolean execute(Transaction transaction, AbstractDataSource.OnErrorListener onErrorListener) {
		// lock the database
		beginLock();
	...
	/**
	 * <p>
	 * Executes a transaction in async mode. This method <strong>is thread
	 * safe</strong> to avoid concurrent problems. The drawback is only one
	 * transaction at time can be executed. The database will be open in write
	 * mode. This method uses default error listener to intercept errors.
	 * </p>
	 *
	 * @param transaction
	 *            transaction to execute
	...
	/**
	 * <p>
	 * Executes a batch command in async mode. This method <strong>is thread
	 * safe</strong> to avoid concurrent problems. The drawback is only one
	 * transaction at time can be executed. The database will be open in write
	 * mode. This method uses default error listener to intercept errors.
	 * </p>
	...
	/**
	 * <p>
	 * Executes a batch opening a read only connection. This method <strong>is
	 * thread safe</strong> to avoid concurrent problems.
	 * </p>
	 *
	 * @param commands
	 *            batch to execute
	...
	/**
	 * <p>
	 * Executes a batch. This method <strong>is thread safe</strong> to avoid
	 * concurrent problems. The drawback is only one transaction at time can be
	 * executed. if <code>writeMode</code> is set to false, multiple batch
	 ...
	 *            batch to execute
	 * @param writeMode
	 *            true to open connection in write mode, false to open
	 *            connection in read only mode
	 */
	public <T> T executeBatch(Batch<T> commands, boolean writeMode) {
		// open database in safe mode
		Pair<Boolean, SQLiteDatabase> status=this.openDatabaseThreadSafeMode(writeMode);
	...
	/**
	 * <p>
	 * Build instance. This method can be used only one time, on the application
	 * start.
	 * </p>
	 */
