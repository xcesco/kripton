...
public class DaoPersonImpl extends Dao implements DaoPerson {
	 ...
	 *
	 * <pre>
	 * SELECT id, name, surname FROM person WHERE name like ${name} || '%' LIMIT 20 OFFSET #{DYNAMIC_PAGE_OFFSET}
	 * </pre>
	 *
	...
	public void insert(Person bean) {
		...
		// if PK string, can not overwrite id (with a long) same thing if column
		// type is UNMANAGED (user manage PK)
		bean.id = result;
		if (result > 0) {
			// rx management
