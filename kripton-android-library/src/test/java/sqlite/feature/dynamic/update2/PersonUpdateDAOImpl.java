...
public class PersonUpdateDAOImpl extends Dao implements PersonUpdateDAO {
  ...
  private static SQLiteStatement updateRawPreparedStatement0;

  private static SQLiteStatement updateBeanPreparedStatement1;
   ...
   * @param nameValue
   * 	is used as updated field <strong>nameValue</strong>
   * @param id
   ...
   */
  @Override
  public void updateRaw(String name, String nameValue, long id, String where) {
    if (updateRawPreparedStatement0==null) {
      // generate static SQL for statement
      String _sql="UPDATE person SET name=? WHERE id = ? #{DYNAMIC_WHERE}";
      updateRawPreparedStatement0 = KriptonDatabaseWrapper.compile(_context, _sql);
    }
    KriptonContentValues _contentValues=contentValuesForUpdate(updateRawPreparedStatement0);
    ...
    if (_context.isLogEnabled()) {
      ...
      // display log
      Logger.info("UPDATE person SET name=:name WHERE id = ? #{DYNAMIC_WHERE}");
    ...
    // log section END
    int result = KriptonDatabaseWrapper.updateDelete(updateRawPreparedStatement0, _contentValues);
  }
  ...
  public void updateBean(Person bean, String where) {
    if (updateBeanPreparedStatement1==null) {
      // generate static SQL for statement
      String _sql="UPDATE person SET name=?, surname=?, birth_city=?, birth_day=? WHERE id = ? #{DYNAMIC_WHERE}";
      updateBeanPreparedStatement1 = KriptonDatabaseWrapper.compile(_context, _sql);
    }
    KriptonContentValues _contentValues=contentValuesForUpdate(updateBeanPreparedStatement1);
    ...
    if (_context.isLogEnabled()) {
      ...
      // display log
      Logger.info("UPDATE person SET name=:name, surname=:surname, birth_city=:birth_city, birth_day=:birth_day WHERE id = ? #{DYNAMIC_WHERE}");
    ...
    // log section END
    int result = KriptonDatabaseWrapper.updateDelete(updateBeanPreparedStatement1, _contentValues);
  }
  ...
  public static void clearCompiledStatements() {
    if (updateRawPreparedStatement0!=null) {
      updateRawPreparedStatement0.close();
      updateRawPreparedStatement0=null;
    }
    if (updateBeanPreparedStatement1!=null) {
      updateBeanPreparedStatement1.close();
      updateBeanPreparedStatement1=null;
    }
    if (insertOnePreparedStatement2!=null) {
