...
public class Dao2PersonImpl extends Dao implements Dao2Person {
  ...
  public Dao2PersonImpl(BindPerson2DaoFactory daoFactory) {
   ...
   * @return result list
   */
  private List<Person> selectWithPageRequest(int pageSize, PageRequest pageRequest) {
    // common part generation - BEGIN
    KriptonContentValues _contentValues=contentValues();
    StringBuilder _sqlBuilder=sqlBuilder();
    _sqlBuilder.append("SELECT id, birth_city, birth_day, name, surname FROM person");
  ...
  public class PaginatedResult3 extends PagedResultImpl<Person> {
    ...
    public List<Person> execute(PageRequest pageRequest) {
      ...
      return BindPerson2DataSource.getInstance().executeBatch(daoFactory -> daoFactory.getDao2Person().selectWithPageRequest(this.pageSize, pageRequest));
