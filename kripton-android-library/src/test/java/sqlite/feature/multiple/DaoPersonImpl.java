...
public class DaoPersonImpl extends Dao implements DaoPerson {
	 ...
	 * </pre>
	 *
	 * <h2>Mapped class:</h2> {@link Person}
	 *
			...
			if (_cursor.moveToFirst()) {
				...
				do {
					...
					if (!_cursor.isNull(index1)) {
						__name = _cursor.getString(index1);
					}
					if (!_cursor.isNull(index2)) {
						__surname = _cursor.getString(index2);
					}
	 ...
	 * 
	 * <pre>
	 * INSERT INTO person (name, surname) VALUES (:bean.name, :bean.surname)
	 * </pre>
	 *
	 * <p>
	 * <code>bean.id</code> is automatically updated because it is the primary key
	 * </p>
	 *
	 * <p>
	...
	public void insert(Person bean) {
		...
		if (_context.isLogEnabled()) {
			...
			Logger.info("INSERT INTO person (%s) VALUES (%s)", _columnNameBuffer.toString(),
					_columnValueBuffer.toString());
			...
			for (int i = 0; i < _contentValues.size(); i++) {
				...
				if (_contentValue.value1 == null) {
					...
					Logger.info("==> :%s = '%s' (%s)", _contentValue.value0,
							StringUtils.checkSize(_contentValue.value1),
							_contentValue.value1.getClass().getCanonicalName());
				}
			}
	 ...
	 * 
	 * <pre>
	 * UPDATE person SET name=:name, surname=:surname WHERE id=${bean.id}
	 * </pre>
	 *
