...
public interface LoanDao {
	...
	@BindSqlSelect(jql = "SELECT Loan.id, Book.title as bookTitle, User.name as userName, Loan.startTime, Loan.endTime " + "FROM Book " + "INNER JOIN Loan ON Loan.bookId = Book.id "
			+ "INNER JOIN User on User.id = Loan.userId " + "WHERE User.name LIKE :userName " + "AND Loan.endTime > :after ")
