...
public class BindAppDataSource extends AbstractDataSource implements BindAppDaoFactory, AppDataSource {
   ...
   */
  public static final int LOAN_DAO_UID = 1;

  /**
   * Unique identifier for Dao UserDao
   */
  public static final int USER_DAO_UID = 2;

  /**
   * List of tables compose datasource
   */
  static final SQLiteTable[] TABLES = {new BookTable(), new UserTable(), new LoanTable()};
  ...
  class DataSourceSingleThread implements BindAppDaoFactory {
    ...
    protected void onSessionClosed() {
      ...
      if (_bookDao!=null && daosWithEvents.contains(BOOK_DAO_UID)) {
        _bookDao.invalidateLiveData();
      }
      if (_loanDao!=null && daosWithEvents.contains(LOAN_DAO_UID)) {
        _loanDao.invalidateLiveData();
      }
      // "_userDao" has no live data
    }
