...
public class LoanDaoImpl extends Dao implements LoanDao {
   ...
   * <h2>Select SQL:</h2>
   *
   * <pre>SELECT loan.id, book.title as book_title, user.name as user_name, loan.start_time, loan.end_time From loan INNER JOIN book ON loan.book_id = book.id INNER JOIN user ON loan.user_id = user.id </pre>
   *
   * <h2>Mapped class:</h2>
   ...
   * <dl>
   * 	<dt>id</dt><dd>is associated to bean's property <strong>id</strong></dd>
   * 	<dt>book_title</dt><dd>is associated to bean's property <strong>bookTitle</strong></dd>
  ...
  protected List<LoanWithUserAndBook> findAllWithUserAndBookForLiveData() {
    ...
    try (Cursor _cursor = database().rawQuery(_sql, _sqlArgs)) {
      ...
      if (_cursor.moveToFirst()) {
        ...
        int index0=_cursor.getColumnIndex("id");
        int index1=_cursor.getColumnIndex("book_title");
        int index2=_cursor.getColumnIndex("user_name");
        int index3=_cursor.getColumnIndex("start_time");
   ...
   * <h2>Select SQL:</h2>
   *
   * <pre>SELECT loan.id, book.title as book_title, user.name as user_name, loan.start_time, loan.end_time FROM book INNER JOIN loan ON loan.book_id = book.id INNER JOIN user on user.id = loan.user_id WHERE user.name LIKE :userName AND loan.end_time > :after </pre>
   *
   * <h2>Mapped class:</h2>
   * {@link LoanWithUserAndBook}
   *
