...
public class DaoArtistImpl extends Dao implements DaoArtist {
   ...
   */
  private static final String SELECT_DATA_BY_ID_SQL6 = "SELECT data FROM artist WHERE id=?";
   ...
   */
  @Override
  public Optional<byte[]> selectDataById(long id) {
    // common part generation - BEGIN
    KriptonContentValues _contentValues=contentValues();
    // query SQL is statically defined
    String _sql=SELECT_DATA_BY_ID_SQL6;
    // add where arguments
    _contentValues.addWhereArgs(String.valueOf(id));
    String[] _sqlArgs=_contentValues.whereArgsAsArray();
    // log section for select BEGIN
