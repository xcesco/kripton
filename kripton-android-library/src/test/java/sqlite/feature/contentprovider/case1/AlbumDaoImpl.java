...
public class AlbumDaoImpl extends Dao implements AlbumDao {
   ...
   */
  protected List<Album> selectAllForLiveData() {
    // common part generation - BEGIN
    KriptonContentValues _contentValues=contentValues();
    // query SQL is statically defined
   ...
   */
  @Override
  public LiveData<List<Album>> selectAll() {
    // common part generation - BEGIN
    // common part generation - END
    final KriptonXLiveDataHandlerImpl<List<Album>> builder=new KriptonXLiveDataHandlerImpl<List<Album>>() {
      @Override
      protected List<Album> compute() {
        return BindArtistsDataSource.getInstance().executeBatch(new BindArtistsDataSource.Batch<List<Album>>() {
          @Override
          public List<Album> onExecute(BindArtistsDaoFactory daoFactory) {
            return daoFactory.getAlbumDao().selectAllForLiveData();
          }
        });
      }
    };
  ...
  protected void registryEvent(int affectedRows) {
    ...
    if (_context.isInSession()) {
      _context.registrySQLEvent(BindArtistsDataSource.ALBUM_DAO_UID);
    } else {
      invalidateLiveData();
    }
  }
