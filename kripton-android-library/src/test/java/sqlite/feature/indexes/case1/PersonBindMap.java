...
public class PersonBindMap extends AbstractMapper<Person> {
  ...
  public int serializeOnJackson(Person object, JsonGenerator jacksonSerializer) throws Exception {
    ...
    // field aliasName (mapped with "aliasName")
    if (object.aliasName!=null)  {
      fieldCount++;
      jacksonSerializer.writeStringField("aliasName", object.aliasName);
    }
  ...
  public void serializeOnXml(Person object, XMLSerializer xmlSerializer, EventType currentEventType)
    ...
    // field aliasName (mapped with "aliasName")
    if (object.aliasName!=null) {
      xmlSerializer.writeStartElement("aliasName");
      xmlSerializer.writeCharacters(StringEscapeUtils.escapeXml10(object.aliasName));
      ...
      switch (fieldName) {
          case "aliasName":
            ...
            if (jacksonParser.currentToken()!=JsonToken.VALUE_NULL) {
              instance.aliasName=jacksonParser.getText();
            }
          break;
          case "birthCity":
            // field birthCity (mapped with "birthCity")
    ...
    while (xmlParser.hasNext() && elementName!=null) {
      ...
      switch(eventType) {
          case START_TAG:
            ...
            switch(currentTag) {
                case "aliasName":
                  // property aliasName (mapped on "aliasName")
                  instance.aliasName=StringEscapeUtils.unescapeXml(xmlParser.getElementText());
                break;
                case "birthCity":
