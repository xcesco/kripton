...
public class BindAppPreferences extends AbstractSharedPreference {
  ...
  private List<Pair<String, WeakReference<MutableLiveData>>> liveDatas = Collections.synchronizedList(new ArrayList<Pair<String, WeakReference<MutableLiveData>>>());
  ...
  @SuppressWarnings("rawtypes")
  protected void registryLiveData(String key, MutableLiveData<?> value) {
    liveDatas.add(new Pair<String , WeakReference<MutableLiveData>>(key, new WeakReference<MutableLiveData>(value)));
  }

  @SuppressWarnings({ "unchecked", "rawtypes" })
  protected void updateLiveData(String key, Object value) {
    for (Pair<String, WeakReference<MutableLiveData>> item : liveDatas) {
      if (item.value0.equals(key) && item.value1.get() != null) {
        item.value1.get().postValue(value);
      }
    }
  }
