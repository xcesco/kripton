...
/**
 * This class is the shared preference binder defined for BeanWithAccessors
 *
 * @see BeanWithAccessors
 */
public class BindBeanWithAccessorsSharedPreferences extends AbstractSharedPreference {
  /**
   * instance of shared preferences
   */
  private static BindBeanWithAccessorsSharedPreferences instance;

  /**
   * working instance of bean
   */
  private final BeanWithAccessors defaultBean;

  /**
   * constructor
   */
  private BindBeanWithAccessorsSharedPreferences() {
    createPrefs();
    defaultBean=new BeanWithAccessors();
  }
   ...
   */
  public BindBeanWithAccessorsSharedPreferences refresh() {
    createPrefs();
    return this;
  }
   ...
   * @return read bean
   */
  public BeanWithAccessors read() {
    BeanWithAccessors bean=new BeanWithAccessors();
     {
      String temp=prefs.getString("local_date_time", null);
      bean.setLocalDateTime((StringUtils.hasText(temp)) ? LocalDateTimeUtils.read(temp): defaultBean.getLocalDateTime());}

     {
      String temp=prefs.getString("zone_offset", null);
      bean.setZoneOffset((StringUtils.hasText(temp)) ? ZoneOffsetUtils.read(temp): defaultBean.getZoneOffset());}

     {
      String temp=prefs.getString("period", null);
      bean.setPeriod((StringUtils.hasText(temp)) ? PeriodUtils.read(temp): defaultBean.getPeriod());}

     {
      String temp=prefs.getString("offset_time", null);
      bean.setOffsetTime((StringUtils.hasText(temp)) ? OffsetTimeUtils.read(temp): defaultBean.getOffsetTime());}

     {
      String temp=prefs.getString("year", null);
      bean.setYear((StringUtils.hasText(temp)) ? YearUtils.read(temp): defaultBean.getYear());}

     {
      String temp=prefs.getString("year_month", null);
      bean.setYearMonth((StringUtils.hasText(temp)) ? YearMonthUtils.read(temp): defaultBean.getYearMonth());}

     {
      String temp=prefs.getString("month_day", null);
      bean.setMonthDay((StringUtils.hasText(temp)) ? MonthDayUtils.read(temp): defaultBean.getMonthDay());}

     {
      String temp=prefs.getString("istant", null);
      bean.setIstant((StringUtils.hasText(temp)) ? InstantUtils.read(temp): defaultBean.getIstant());}

     {
      String temp=prefs.getString("duration", null);
      bean.setDuration((StringUtils.hasText(temp)) ? DurationUtils.read(temp): defaultBean.getDuration());}

     {
      String temp=prefs.getString("local_time", null);
      bean.setLocalTime((StringUtils.hasText(temp)) ? LocalTimeUtils.read(temp): defaultBean.getLocalTime());}

     {
      String temp=prefs.getString("zoned_date_time", null);
      bean.setZonedDateTime((StringUtils.hasText(temp)) ? ZonedDateTimeUtils.read(temp): defaultBean.getZonedDateTime());}

     {
      String temp=prefs.getString("zone_id", null);
      bean.setZoneId((StringUtils.hasText(temp)) ? ZoneIdUtils.read(temp): defaultBean.getZoneId());}

     {
      String temp=prefs.getString("offset_date_time", null);
      bean.setOffsetDateTime((StringUtils.hasText(temp)) ? OffsetDateTimeUtils.read(temp): defaultBean.getOffsetDateTime());}

     {
      String temp=prefs.getString("local_date", null);
      bean.setLocalDate((StringUtils.hasText(temp)) ? LocalDateUtils.read(temp): defaultBean.getLocalDate());}
   ...
   */
  public void write(BeanWithAccessors bean) {
   ...
   */
  public static synchronized BindBeanWithAccessorsSharedPreferences getInstance() {
    if (instance==null) {
      instance=new BindBeanWithAccessorsSharedPreferences();
