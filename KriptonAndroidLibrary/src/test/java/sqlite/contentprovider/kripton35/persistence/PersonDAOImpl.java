...
public class PersonDAOImpl extends AbstractDao implements PersonDAO {
	 ...
	 * 
	 * <pre>
	 * INSERT OR FAIL INTO person (birth_city, birth_day, name, surname) VALUES (${bean.birthCity}, ${bean.birthDay}, ${bean.name}, ${bean.surname})
	 * </pre>
	 *
	 * <p>
	 ...
	 * <dl>
	 * <dt>birth_city</dt>
	 * <dd>is mapped to <strong>${bean.birthCity}</strong></dd>
	 * <dt>birth_day</dt>
	 * <dd>is mapped to <strong>${bean.birthDay}</strong></dd>
	 * <dt>name</dt>
	 * <dd>is mapped to <strong>${bean.name}</strong></dd>
	 * <dt>surname</dt>
	 * <dd>is mapped to <strong>${bean.surname}</strong></dd>
	 * </dl>
	 *
	 * @param bean
	...
	public void insertOne(Person bean) {
		...
		Logger.info(SqlUtils.formatSQL("INSERT OR FAIL INTO person (birth_city, birth_day, name, surname) VALUES ('" + StringUtils.checkSize(contentValues.get("birth_city")) + "', '"
				+ StringUtils.checkSize(contentValues.get("birth_day")) + "', '" + StringUtils.checkSize(contentValues.get("name")) + "', '" + StringUtils.checkSize(contentValues.get("surname"))
				+ "')"));
	...
	static Set<String> insertOneContentCheckerColumns = CollectionUtils.newSet("a", "a");

	void insertOneContentChecker(ContentValues contentValues) {
		// INSERT OR FAIL INTO Person (birthCity, birthDay, name, surname)
		// VALUES (${bean.birthCity}, ${bean.birthDay}, ${bean.name},
		// ${bean.surname})
		// INSERT OR FAIL INTO Person (birthCity, birthDay, name, surname)
		// VALUES (arturo, arturo, arturo, arturo)
	...
	long insertOne(ContentValues contentValues) {
		// INSERT OR FAIL INTO Person (birthCity, birthDay, name, surname)
		// VALUES (${bean.birthCity}, ${bean.birthDay}, ${bean.name},
		// ${bean.surname})
		// INSERT OR FAIL INTO Person (birthCity, birthDay, name, surname)
		// VALUES (arturo, arturo, arturo, arturo)
		insertOneContentChecker(contentValues);
	 ...
	 * 
	 * <pre>
	 * UPDATE person SET birthCity=${birthCity} WHERE id=${id} #{where}
	 * </pre>
	 *
	 * <h2>Updated columns:</strong></h2>
	 * <dl>
	 * <dt>birth_city</dt>
	 * <dd>is binded to query's parameter <strong>${birthCity}</strong> and
	 * method's parameter <strong>dummy</strong></dd>
	 * </dl>
	 *
	 * <h2>Where parameters:</h2>
	 * <dl>
	 * <dt>${id}</dt>
	 * <dd>is mapped to method's parameter <strong>id</strong></dd>
	 * </dl>
	 *
	 * <h2>Dynamic parts:</h2>
	 * <dl>
	 * <dt>#{where}</dt>
	 * <dd>is part of where conditions resolved at runtime.</dd>
	 * </dl>
	 *
	 * @param dummy
	...
	public int updateWhereStaticAndDynamic(String dummy, long id, String where) {
		...
		// StringUtils and SqlUtils will be used to format SQL
		Logger.info(SqlUtils.formatSQL("UPDATE person SET birthCity='" + StringUtils.checkSize(contentValues.get("birth_city")) + "' WHERE id=%s " + SqlUtils.appendForLog(where),
				(Object[]) whereConditionsArray));
	 ...
	 *
	 * <pre>
	 * SELECT id, birth_city, birth_day, name, surname FROM person WHERE name like ${nameTemp} || \'%\' #{where} HAVING id=2 GROUP BY id ORDER BY id#{orderBy}#{pageSize}
	 * </pre>
	 *
	 ...
	 * <dt>birth_city</dt>
	 * <dd>is associated to bean's property <strong>birthCity</strong></dd>
	 * <dt>birth_day</dt>
	 * <dd>is associated to bean's property <strong>birthDay</strong></dd>
	 * <dt>name</dt>
	 ...
	 * <dl>
	 * <dt>${nameTemp}</dt>
	 * <dd>is binded to method's parameter <strong>nameValue</strong></dd>
	 * </dl>
	 *
	 * @param nameValue
	 *            is binded to <code>${nameTemp}</code>
	 * @param pageSize
	 *            is used as <strong>dynamic LIMIT statement</strong> and it is
	 *            formatted by ({@link StringUtils#format})
	 * @param where
	 *            is used as <strong>dynamic WHERE statement</strong> and it is
	 *            formatted by ({@link StringUtils#format})
	 * @param orderBy
	 *            is used as <strong>dynamic ORDER BY statement</strong> and it
	 *            is formatted by ({@link StringUtils#format})
	...
	public List<Person> selectOne(String nameValue, int pageSize, String where, String orderBy) {
		...
		Logger.info(SqlUtils.formatSQL("SELECT id, birth_city, birth_day, name, surname FROM person WHERE name like '%s' || \'%%' " + SqlUtils.appendForLog(where)
				+ " HAVING id=2 GROUP BY id ORDER BY id" + SqlUtils.appendForLog(orderBy) + SqlUtils.printIf(pageSize > 0, " LIMIT " + pageSize), (Object[]) _args));
		try (Cursor cursor = database().rawQuery("SELECT id, birth_city, birth_day, name, surname FROM person WHERE name like ? || \'%\' " + SqlUtils.appendForSQL(where)
				+ " HAVING id=2 GROUP BY id ORDER BY id" + SqlUtils.appendForSQL(orderBy) + SqlUtils.printIf(pageSize > 0, " LIMIT " + pageSize), _args)) {
			...
			if (cursor.moveToFirst()) {
				...
				do {
					...
					if (!cursor.isNull(index1)) {
						resultBean.birthCity = cursor.getString(index1);
					}
					if (!cursor.isNull(index2)) {
						resultBean.birthDay = DateUtils.read(cursor.getString(index2));
					}
					if (!cursor.isNull(index3)) {
						resultBean.setName(cursor.getString(index3));
					}
					if (!cursor.isNull(index4)) {
						resultBean.setSurname(cursor.getString(index4));
					}
	 ...
	 *
	 * <pre>
	 * SELECT id, birth_city, birth_day, name, surname FROM person ORDER BY name#{orderBy}
	 * </pre>
	 *
	 * <h2>Projected columns:</h2>
	 * <dl>
	 ...
	 * 
	 * <pre>
	 * DELETE person WHERE id = ${bean.id} #{where}
	 * </pre>
	 *
	 * <h2>Parameters used in where conditions:</h2>
	 * <dl>
	 * <dt>${bean.id}</dt>
	 * <dd>is mapped to method's parameter <strong>bean.id</strong></dd>
	 * </dl>
	 *
	 * <h2>Dynamic parts:</h2>
	 * <dl>
