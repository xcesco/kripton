...
public class UpdateBeanPersonDaoImpl extends AbstractDao implements UpdateBeanPersonDao {
  ...
  private static final Set<String> updateAllBeansJQL4ColumnSet = CollectionUtils.asSet(String.class, "name", "surname", "student");

  private static final Set<String> updateBeanDynamic5ColumnSet = CollectionUtils.asSet(String.class, "name", "surname", "student");

  private static final Set<String> updateBeanDynamicWithArgs6ColumnSet = CollectionUtils.asSet(String.class, "name", "surname", "student");
   ...
   * @return number of effected rows
   */
  int updateBeanDynamic5(Uri uri, ContentValues contentValues, String selection,
      String[] selectionArgs) {
    Logger.info("Execute UPDATE for URI %s", uri.toString());
    StringBuilder _sqlBuilder=new StringBuilder();
    // generation CODE_001 -- BEGIN
    ...
    _sqlWhereParams.add(uri.getPathSegments().get(1));
    for (String columnName:contentValues.keySet()) {
      if (!updateBeanDynamic5ColumnSet.contains(columnName)) {
   ...
   * @return number of effected rows
   */
  int updateBeanDynamicWithArgs6(Uri uri, ContentValues contentValues, String selection,
      String[] selectionArgs) {
    Logger.info("Execute UPDATE for URI %s", uri.toString());
    StringBuilder _sqlBuilder=new StringBuilder();
    // generation CODE_001 -- BEGIN
    ...
    for (String columnName:contentValues.keySet()) {
      if (!updateBeanDynamicWithArgs6ColumnSet.contains(columnName)) {
