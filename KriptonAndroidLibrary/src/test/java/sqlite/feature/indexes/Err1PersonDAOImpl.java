...
/**
 * <p>
 * DAO implementation for entity <code>Err1Person</code>, based on interface <code>Err1PersonDAO</code>
 * </p>
 *
 *  @see Err1Person
 *  @see Err1PersonDAO
 *  @see Err1PersonTable
 */
public class Err1PersonDAOImpl extends AbstractDao implements Err1PersonDAO {
  public Err1PersonDAOImpl(BindErr1PersonDataSource dataSet) {
    super(dataSet);
  }

  /**
   * <h2>SQL insert</h2>
   * <pre>INSERT INTO err1_person (name, surname, birth_city, birth_day) VALUES (${name}, ${surname}, ${birthCity}, ${birthDay})</pre>
   *
  ...
  public void insertOne(String name, String surname, String birthCity, Date birthDay) {
    ...
    Logger.info("INSERT INTO err1_person (%s) VALUES (%s)", _columnNameBuffer.toString(), _columnValueBuffer.toString());
    ...
    database().insert("err1_person", null, contentValues);
  }

  /**
   * <h2>Select SQL:</h2>
   *
   * <pre>SELECT id, name, surname, birth_city, birth_day FROM err1_person ORDER BY name</pre>
   *
   ...
   */
  @Override
  public List<Err1Person> selectAll() {
    StringBuilder _sqlBuilder=getSQLStringBuilder();
    _sqlBuilder.append("SELECT id, name, surname, birth_city, birth_day FROM err1_person");
    ...
    try (Cursor cursor = database().rawQuery(_sql, _sqlArgs)) {
      ...
      LinkedList<Err1Person> resultList=new LinkedList<Err1Person>();
      Err1Person resultBean=null;
   ...
   * <h2>Select SQL:</h2>
   *
   * <pre>SELECT id, name, surname, birth_city, birth_day FROM err1_person WHERE name like ${nameTemp} || '%' AND #{DYNAMIC_WHERE} ORDER BY #{DYNAMIC_ORDER_BY}</pre>
   *
   * <h2>Projected columns:</h2>
   * <dl>
   * 	<dt>id</dt><dd>is associated to bean's property <strong>id</strong></dd>
   ...
   */
  @Override
  public List<Err1Person> selectOne(String nameValue, String where, String orderBy) {
    StringBuilder _sqlBuilder=getSQLStringBuilder();
    _sqlBuilder.append("SELECT id, name, surname, birth_city, birth_day FROM err1_person");
    // generation CODE_001 -- BEGIN
   ...
   * <h2>Select SQL:</h2>
   *
   * <pre>SELECT id, name, surname, birth_city, birth_day FROM err1_person ORDER BY name, #{DYNAMIC_ORDER_BY}</pre>
   *
   * <h2>Projected columns:</h2>
   * <dl>
   * 	<dt>id</dt><dd>is associated to bean's property <strong>id</strong></dd>
   ...
   *
   * @param beanListener
   * 	is the Err1Person listener
   * @param orderBy
   * 	is used as <strong>dynamic ORDER BY statement</strong> and it is formatted by ({@link StringUtils#format})
   */
  @Override
  public void selectBeanListener(OnReadBeanListener<Err1Person> beanListener, String orderBy) {
    StringBuilder _sqlBuilder=getSQLStringBuilder();
    _sqlBuilder.append("SELECT id, name, surname, birth_city, birth_day FROM err1_person");
    // generation CODE_001 -- BEGIN
    // generation CODE_001 -- END
    ...
    try (Cursor cursor = database().rawQuery(_sql, _sqlArgs)) {
      Logger.info("Rows found: %s",cursor.getCount());
      Err1Person resultBean=new Err1Person();
      if (cursor.moveToFirst()) {
