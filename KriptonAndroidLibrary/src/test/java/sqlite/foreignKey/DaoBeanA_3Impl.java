...
public class DaoBeanA_3Impl extends AbstractDao implements DaoBeanA_3 {
   ...
   * <h2>Select SQL:</h2>
   *
   * <pre>SELECT id, valueString2 FROM bean_a_3</pre>
   *
  ...
  public List<BeanA_3> selectAll() {
    ...
    _sqlBuilder.append("SELECT pk, value_string2 FROM bean_a_3");
   ...
   * <h2>Select SQL:</h2>
   *
   * <pre>SELECT id, valueString2 FROM bean_a_3 WHERE id=${id}</pre>
   *
   * <h2>Projected columns:</h2>
   * <dl>
   * 	<dt>pk</dt><dd>is associated to bean's property <strong>id</strong></dd>
  ...
  public List<BeanA_3> selectByString(String value) {
    StringBuilder _sqlBuilder=new StringBuilder();
    _sqlBuilder.append("SELECT pk FROM bean_a_3 ");
    // generation CODE_001 -- BEGIN
    // generation CODE_001 -- END
    ArrayList<String> _sqlWhereParams=new ArrayList<>();
  ...
  /**
   * <h2>SQL update:</h2>
   * <pre>UPDATE bean_a_3 SET value_string2=${bean.valueString2} WHERE WHERE valueString2=${bean.valueString2}</pre>
   *
  ...
  public int update(BeanA_3 bean) {
    ...
    // display log
    Logger.info("UPDATE bean_a_3 SET valueString2=:value_string2 WHERE valueString2=?");
    ...
    // log for where parameters -- END
    int result = database().update("bean_a_3", contentValues, _sqlWhereStatement, _sqlWhereParams.toArray(new String[_sqlWhereParams.size()]));;
    return result;
  }
}
