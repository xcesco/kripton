...
public class BeanDao2Impl extends AbstractDao implements BeanDao2 {
  ...
  public List<Bean2> selectList(long id) {
    ...
    try (Cursor cursor = database().rawQuery(_sql, _sqlArgs)) {
      ...
      ArrayList<Bean2> resultList=new ArrayList<Bean2>(cursor.getCount());
      Bean2 resultBean=null;
  ...
  public long updateOne(Bean2 value) {
    if (updateOnePreparedStatement0==null) {
      ...
      String _sql="UPDATE bean2 SET value=?, valueByteSet=?, valueShortSet=?, valueIntegerSet=?, valueStringSet=?, valueCharacterSet=?, valueFloatSet=?, valueDoubleSet=?, valueBigDecimalSet=?, valueBeanSet=?, valueEnumTypeSet=? WHERE id=?";
