...
public abstract class ConfigBase {
	...
	public static boolean hashPackManagerDependencies(String clazzName) {
		boolean packManagerDependencies;
		Field[] fields = null;
		try {
			fields = Class.forName(clazzName).getFields();
		} catch (ClassNotFoundException e) {						
			Logger.fatal(e.getMessage());
			e.printStackTrace();
		}

		packManagerDependencies = false;

		// mettiamo in annotatedFields tutti i campi annotation
		for (Field field : fields) {

			if (field.getType().isAssignableFrom(PackItem.class) || field.getType().isAssignableFrom(PackResource.class)
					|| (field.getType().isArray() && (field.getType().getComponentType().isAssignableFrom(PackItem.class) || field.getType().getComponentType().isAssignableFrom(PackResource.class)))) {
				packManagerDependencies = true;
			}
		}

		return packManagerDependencies;
	}
