...
public class BeanDaoImpl extends AbstractDao implements BeanDao {
  ...
  public BeanBean selectOne() {
    ...
    if (cursor.moveToFirst()) {
      ...
      if (!cursor.isNull(index1)) { ArrayList<BeanInner> collection=ProcessorHelper.asCollection(new ArrayList<BeanInner>(), BeanInner.class, cursor.getBlob(index1)); resultBean.setValue(CollectionUtility.asArray(collection, new BeanInner[collection.size()])); }
      if (!cursor.isNull(index2)) { ArrayList<BeanInner> collection=ProcessorHelper.asCollection(new ArrayList<BeanInner>(), BeanInner.class, cursor.getBlob(index2)); resultBean.setValue2(CollectionUtility.asArray(collection, new BeanInner[collection.size()])); }
