...
public class BindBeanDataSource extends AbstractDataSource implements BindBeanDaoFactory, BeanDataSource {
  ...
  public void onCreate(SQLiteDatabase database) {
    // generate tables
    Logger.info("DDL: %s",Bean63$Table.CREATE_TABLE_SQL);
    database.execSQL(Bean63$Table.CREATE_TABLE_SQL);
  }
  ...
  public void onUpgrade(SQLiteDatabase database, int oldVersion, int newVersion) {
    // drop tables
    Logger.info("DDL: %s",Bean63$Table.DROP_TABLE_SQL);
    database.execSQL(Bean63$Table.DROP_TABLE_SQL);
