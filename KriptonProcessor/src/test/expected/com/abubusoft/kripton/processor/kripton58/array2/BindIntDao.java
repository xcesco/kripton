...
public class BindIntDao extends AbstractDao implements IntDao {
  ...
  public IntBean selectOne() {
    ...
    if (cursor.moveToFirst()) {
      ...
      if (!cursor.isNull(index1)) { resultBean.setValue(CollectionUtility.toIntegerTypeArray(DaoHelper.toList(Integer.TYPE, cursor.getBlob(index1)))); }
      if (!cursor.isNull(index2)) { resultBean.setValue2(CollectionUtility.toIntegerArray(DaoHelper.toList(Integer.class, cursor.getBlob(index2)))); }
