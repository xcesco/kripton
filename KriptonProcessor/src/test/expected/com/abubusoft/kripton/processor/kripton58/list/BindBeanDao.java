...
public class BindBeanDao extends AbstractDao implements BeanDao {
  ...
  public BeanBean selectOne() {
    ...
    if (cursor.moveToFirst()) {
      ...
      if (!cursor.isNull(index1)) { resultBean.value=DaoHelper.toList(new ArrayList<BeanInner>(), BeanInner.class, cursor.getBlob(index1)); }
      if (!cursor.isNull(index2)) { resultBean.value2=DaoHelper.toList(new LinkedList<BeanInner>(), BeanInner.class, cursor.getBlob(index2)); }
