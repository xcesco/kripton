...
public class ShortDaoImpl extends AbstractDao implements ShortDao {
  ...
  public ShortBean selectOne() {
    ...
    if (cursor.moveToFirst()) {
      ...
      if (!cursor.isNull(index1)) { resultBean.value=CollectionUtils.asShortTypeArray(ProcessorHelper.asList(Short.TYPE, cursor.getBlob(index1))); }
      if (!cursor.isNull(index2)) { resultBean.value2=CollectionUtils.asShortArray(ProcessorHelper.asList(Short.class, cursor.getBlob(index2))); }
