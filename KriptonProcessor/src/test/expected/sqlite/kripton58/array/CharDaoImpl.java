...
public class CharDaoImpl extends AbstractDao implements CharDao {
  ...
  public CharBean selectOne() {
    ...
    if (cursor.moveToFirst()) {
      ...
      if (!cursor.isNull(index1)) { resultBean.value=CollectionUtils.asCharacterTypeArray(ProcessorHelper.asList(Character.TYPE, cursor.getBlob(index1))); }
      if (!cursor.isNull(index2)) { resultBean.value2=CollectionUtils.asCharacterArray(ProcessorHelper.asList(Character.class, cursor.getBlob(index2))); }
