...
public class FloatDaoImpl extends AbstractDao implements FloatDao {
  ...
  public FloatBean selectOne() {
    ...
    if (cursor.moveToFirst()) {
      ...
      if (!cursor.isNull(index1)) { resultBean.value=CollectionUtils.asFloatTypeArray(ProcessorHelper.asList(Float.TYPE, cursor.getBlob(index1))); }
      if (!cursor.isNull(index2)) { resultBean.value2=CollectionUtils.asFloatArray(ProcessorHelper.asList(Float.class, cursor.getBlob(index2))); }
