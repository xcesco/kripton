...
public class ShortDaoImpl extends AbstractDao implements ShortDao {
  ...
  public ShortBean selectOne() {
    ...
    if (cursor.moveToFirst()) {
      ...
      if (!cursor.isNull(index1)) { resultBean.value=ProcessorHelper.asCollection(new ArrayList<Short>(), Short.class, cursor.getBlob(index1)); }
      if (!cursor.isNull(index2)) { resultBean.value2=ProcessorHelper.asCollection(new LinkedList<Short>(), Short.class, cursor.getBlob(index2)); }
  ...
  public long insert(long id, List<Short> value) {
    ...
    // log
    Logger.info(StringUtils.formatSQL("SQL: INSERT INTO short_bean (id, value) VALUES ('"+StringUtils.checkSize(contentValues.get("id"))+"', '"+StringUtils.checkSize(contentValues.get("value"))+"')"));
