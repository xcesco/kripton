...
/**
 * <p>
 * DAO implementation for entity <code>Bean01</code>, based on interface
 * <code>DaoBean01</code>
 * </p>
 *
 * @see Bean01
...
public class DaoBean01Impl extends AbstractDao implements DaoBean01 {
	 ...
	 * 
	 * <pre>
	 * SELECT temp, id, text FROM bean01 WHERE id=${id}
	 * </pre>
	 *
	 ...
	 * <dl>
	 * <dt>temp</dt>
	 * <dd>is associated to bean's property <strong>temp</strong></dd>
	 * <dt>id</dt>
	 * <dd>is associated to bean's property <strong>id</strong></dd>
	 * <dt>text</dt>
	 * <dd>is associated to bean's property <strong>text</strong></dd>
	 * </dl>
	 *
		...
		try (Cursor cursor = database().rawQuery("SELECT temp, id, text FROM bean01 WHERE id=?", args)) {
			...
			if (cursor.moveToFirst()) {
				...
				if (!cursor.isNull(index0)) {
					resultBean.temp = Bean01Table.parseTemp(cursor.getBlob(index0));
				}
				if (!cursor.isNull(index1)) {
					resultBean.setId(cursor.getLong(index1));
				}
				if (!cursor.isNull(index2)) {
					resultBean.setText(cursor.getString(index2));
				}
	 ...
	 *
	 * <p>
	 * <strong>Updated columns:</strong>
	 * </p>
	 * <dl>
	 * <dt>text</dt>
	 * <dd>is binded to query's parameter <strong>${text}</strong> and method's
	 * parameter <strong>text</strong></dd>
	 * </dl>
	 *
	 * <p>
	 * <strong>Where parameters:</strong>
	 * </p>
	 * <dl>
	 * <dt>${id}</dt>
	...
	public long updateOne(String text, long id) {
		...
		Logger.info(StringUtils.formatSQL(
				"UPDATE bean01 SET text='" + StringUtils.checkSize(contentValues.get("text")) + "' WHERE id=%s"),
