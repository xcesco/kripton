...
public class Bean73BindMap extends AbstractMapper<Bean73> {
  /**
   * Bean73BindMap */
  private Bean73BindMap bean73BindMap;
  ...
  private Bean73BindMap bean73BindMap() {
    if (bean73BindMap==null) {
      bean73BindMap=AbstractContext.mapperFor(Bean73.class);
    }
    return bean73BindMap;
  }
   ...
   */
  @Override
  public int serializeOnJackson(AbstractJacksonContext context, Bean73 object, JacksonWrapperSerializer wrapper) {
    try {
      ...
      if (object.valueBeanArray!=null)  {
        ...
        for (int i=0; i<n; i++) {
          ...
          if (item==null) {
            ...
            bean73BindMap().serializeOnJackson(context, item, wrapper);
          }
        }
   ...
   */
  @Override
  public int serializeOnJacksonAsString(AbstractJacksonContext context, Bean73 object, JacksonWrapperSerializer wrapper) {
    try {
      JsonGenerator jacksonSerializer = wrapper.jacksonGenerator;
      jacksonSerializer.writeStartObject();
      int fieldCount=0;
        ...
        if (n>0) {
          ...
          for (int i=0; i<n; i++) {
            ...
            if (item==null) {
              ...
              if (bean73BindMap().serializeOnJacksonAsString(context, item, wrapper)==0) {
   ...
   */
  @Override
  public void serializeOnXml(KriptonXmlContext context, Bean73 object, XmlWrapperSerializer wrapper, int currentEventType) {
    try {
      ...
      if (object.valueBeanArray!=null)  {
        ...
        for (int i=0; i<n; i++) {
          ...
          if (item==null) {
            ...
            bean73BindMap().serializeOnXml(context, item, wrapper, 2);
            xmlSerializer.writeEndElement();
          }
        }
        xmlSerializer.writeEndElement();
   ...
   */
  @Override
  public Bean73 parseOnJackson(AbstractJacksonContext context, JacksonWrapperParser wrapper) {
    try {
              ...
              if (jacksonParser.currentToken()==JsonToken.START_ARRAY) {
                ...
                while (jacksonParser.nextToken() != JsonToken.END_ARRAY) {
                  if (jacksonParser.currentToken()==JsonToken.VALUE_NULL) {
                    ...
                    item=bean73BindMap().parseOnJackson(context, wrapper);
                  }
                  collection.add(item);
                }
   ...
   */
  @Override
  public Bean73 parseOnJacksonAsString(AbstractJacksonContext context, JacksonWrapperParser wrapper) {
    try {
      JsonParser jacksonParser = wrapper.jacksonParser;
      Bean73 instance = createInstance();
      String fieldName;
              ...
              if (jacksonParser.currentToken()==JsonToken.START_ARRAY) {
                ...
                while (jacksonParser.nextToken() != JsonToken.END_ARRAY) {
                  ...
                  if (jacksonParser.currentToken()==JsonToken.VALUE_STRING && "null".equals(tempValue)) {
                    ...
                    item=bean73BindMap().parseOnJacksonAsString(context, wrapper);
                  }
                  collection.add(item);
                }
                instance.valueBeanArray=CollectionUtils.asArray(collection, new Bean73[collection.size()]);
   ...
   */
  @Override
  public Bean73 parseOnXml(KriptonXmlContext context, XmlWrapperParser wrapper, int currentEventType) {
    try {
                    ...
                    // property valueBeanArray (mapped on "valueBeanArray")
                     {
                      ...
                      while (xmlParser.nextTag() != XmlPullParser.END_TAG && xmlParser.getName().toString().equals("item")) {
                        if (xmlParser.isEmptyElement()) {
                          ...
                          item=bean73BindMap().parseOnXml(context, wrapper, eventType);
                        }
                        collection.add(item);
                      }
                      instance.valueBeanArray=CollectionUtils.asArray(collection, new Bean73[collection.size()]);
