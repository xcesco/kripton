...
public class Bean81RBindMap extends AbstractMapper<Bean81R> {
   ...
   */
  @Override
  public int serializeOnJackson(JacksonContext context, Bean81R object, JacksonWrapperSerializer wrapper) {
    try {
   ...
   */
  @Override
  public int serializeOnJacksonAsString(JacksonContext context, Bean81R object, JacksonWrapperSerializer wrapper) {
    try {
      JsonGenerator jacksonSerializer = wrapper.jacksonGenerator;
      jacksonSerializer.writeStartObject();
      int fieldCount=0;
   ...
   */
  @Override
  public void serializeOnXml(XmlBinderContext context, Bean81R object, XmlWrapperSerializer wrapper, int currentEventType) {
    try {
   ...
   */
  @Override
  public Bean81R parseOnJackson(JacksonContext context, JacksonWrapperParser wrapper) {
    try {
   ...
   */
  @Override
  public Bean81R parseOnJacksonAsString(JacksonContext context, JacksonWrapperParser wrapper) {
    try {
      JsonParser jacksonParser = wrapper.jacksonParser;
      Bean81R instance = createInstance();
      String fieldName;
   ...
   */
  @Override
  public Bean81R parseOnXml(XmlBinderContext context, XmlWrapperParser wrapper, int currentEventType) {
    try {
                    ...
                    // property valueMapStringInteger
                     {
                      ...
                      collection.put(key, value);
                      while (xmlParser.nextTag() != XMLEventConstants.END_ELEMENT && xmlParser.getName().toString().equals("valueMapStringInteger")) {
                        attributeIndex=xmlParser.getAttributeIndex(null, "key");
                        key=xmlParser.getAttributeValue(attributeIndex);
                        attributeIndex=xmlParser.getAttributeIndex(null, "value");
                        value=PrimitiveUtils.readInteger(xmlParser.getAttributeValue(attributeIndex), null);
