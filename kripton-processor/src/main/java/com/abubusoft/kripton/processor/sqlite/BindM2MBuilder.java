...
public class BindM2MBuilder extends AbstractBuilder {
	...
	private void generateSelects(M2MEntity entity, String packageName) {
		...
		if (!isMethodAlreadyDefined(entity, methodName)) {
			...
			MethodSpec methodSpec = MethodSpec.methodBuilder(methodName)
					...
					.addAnnotation(AnnotationSpec.builder(BindSqlSelect.class).addMember("where", "$S",entity.idName+"="+SqlAnalyzer.PARAM_PREFIX+entity.idName+SqlAnalyzer.PARAM_SUFFIX).build())
		...
		if (!isMethodAlreadyDefined(entity, methodName)) {
			...
			MethodSpec methodSpec = MethodSpec.methodBuilder(methodName)
					.addModifiers(Modifier.PUBLIC)
					.addModifiers(Modifier.ABSTRACT)
					.addAnnotation(AnnotationSpec.builder(BindSqlSelect.class).addMember("where", "$S",workId+"="+SqlAnalyzer.PARAM_PREFIX+workId+SqlAnalyzer.PARAM_SUFFIX).build())
	...
	private void generateDeletes(M2MEntity entity, String packageName) {
		...
		MethodSpec methodSpec = MethodSpec.methodBuilder(methodName)
				.addModifiers(Modifier.PUBLIC)
				.addModifiers(Modifier.ABSTRACT)
				.addAnnotation(AnnotationSpec.builder(BindSqlDelete.class).addMember("where", "$S",entity.idName+"="+SqlAnalyzer.PARAM_PREFIX+entity.idName+SqlAnalyzer.PARAM_SUFFIX).build())
				.addParameter(ParameterSpec.builder(Long.TYPE, entity.idName)
						.addAnnotation(AnnotationSpec.builder(BindSqlParam.class).addMember("value", "$S",entity.idName).build()).build())
		...
		if (!isMethodAlreadyDefined(entity, methodName)) {
			...
			MethodSpec methodSpec = MethodSpec.methodBuilder(methodName)
					.addModifiers(Modifier.PUBLIC)
					.addModifiers(Modifier.ABSTRACT)
					.addAnnotation(AnnotationSpec.builder(BindSqlDelete.class).addMember("where", "$S",workId+"="+SqlAnalyzer.PARAM_PREFIX+workId+SqlAnalyzer.PARAM_SUFFIX).build())
					.addParameter(ParameterSpec.builder(Long.TYPE, workId)
							.addAnnotation(AnnotationSpec.builder(BindSqlParam.class).addMember("value", "$S",workId).build()).build())
					.returns(Integer.TYPE)				
					.build();
