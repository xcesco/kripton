...
public class BindDataSourceBuilder extends AbstractBuilder {
    ...
    private static void generateSchema(SQLiteDatabaseSchema schema) throws FileNotFoundException, IOException {
        ...
        bw.write("------------------------------------------------------------------------------------\n");
        bw.write("--\n");
        bw.write(String.format("-- Filename: %s%n", schemaCreation));
        if (!BindDataSourceSubProcessor.JUNIT_TEST_MODE) {
            bw.write(String.format("-- Generated by: kripton annotation processor %s%n", KriptonProcessorVersion.VERSION));
        }
        if (!BindDataSourceSubProcessor.JUNIT_TEST_MODE && BindDataSourceSubProcessor.SCHEMA_INCLUDE_DATE_MODE) {
            bw.write(String.format("-- Date: %s%n", new Date()));
        }
