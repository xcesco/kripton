...
public abstract class AbstractSelectCodeGenerator implements SelectCodeGenerator {
	...
	protected void generateSubQueries(Builder methodBuilder, SQLiteModelMethod method) {
		...
		for (Triple<String, String, SQLiteModelMethod> item : method.childrenSelects) {
			...
			if (entity.isImmutablePojo()) {
				...
				setter = ImmutableUtility.IMMUTABLE_PREFIX
						+ entity.findRelationByParentProperty(item.value0).value0.getName() + "="
	...
	public void generate(TypeSpec.Builder classBuilder, boolean mapFields, SQLiteModelMethod method) {
		...
		generateCommonPart(method, classBuilder, methodBuilder, fieldList, selectType.isMapFields(), true);
	...
	public void generateLiveData(TypeSpec.Builder classBuilder, SQLiteModelMethod method) {
		...
		generateCommonPart(method, classBuilder, methodBuilder, fieldList, selectType.isMapFields(),
				GenerationType.NO_CONTENT, method.liveDataReturnClass, true);
		...
		if (pagedLiveData) {
			...
			TypeSpec batchBuilder = TypeSpec.anonymousClassBuilder("")
					...
					.addMethod(MethodSpec.methodBuilder("onExecute").addAnnotation(Override.class)
							...
							.returns(method.getReturnClass()).addStatement("return paginatedResult.execute(daoFactory)")
							.build())
					.build();

			// return
			TypeSpec.Builder liveDataBuilderBuilder = TypeSpec.anonymousClassBuilder("paginatedResult")
					.addSuperinterface(ParameterizedTypeName.get(ClassName.get(handlerClass), method.getReturnClass()))
					.addMethod(MethodSpec.methodBuilder("compute").addAnnotation(Override.class)
							.addModifiers(Modifier.PROTECTED).returns(method.getReturnClass()).addStatement(
									"return $T.getInstance().executeBatch($L)", dataSourceClazz, batchBuilder)
							.build());
			TypeSpec liveDataBuilder = liveDataBuilderBuilder.build();

			methodBuilder.addStatement("final $T builder=$L",
					ParameterizedTypeName.get(ClassName.get(handlerClass), method.getReturnClass()), liveDataBuilder);
		} else {
			TypeSpec batchBuilder = TypeSpec.anonymousClassBuilder("")
					.addSuperinterface(ParameterizedTypeName.get(batchClazz, method.getReturnClass()))
					.addMethod(MethodSpec.methodBuilder("onExecute").addAnnotation(Override.class)
			...
			TypeSpec.Builder liveDataBuilderBuilder = TypeSpec.anonymousClassBuilder("")
					.addSuperinterface(ParameterizedTypeName.get(ClassName.get(handlerClass), method.getReturnClass()))
					.addMethod(MethodSpec.methodBuilder("compute").addAnnotation(Override.class)
							.addModifiers(Modifier.PROTECTED).returns(method.getReturnClass()).addStatement(
									"return $T.getInstance().executeBatch($L)", dataSourceClazz, batchBuilder)
	...
	public void generateCommonPart(SQLiteModelMethod method, TypeSpec.Builder classBuilder,
			MethodSpec.Builder methodBuilder, Set<JQLProjection> fieldList, boolean mapFields,
			GenerationType generationType, TypeName forcedReturnType, boolean generateSqlField,
		...
		if (generationType.generateMethodContent) {
			...
			if (method.hasDynamicParts()) {
				...
				if (generateSqlField) {
					...
					sqlName = CaseFormat.LOWER_CAMEL.to(CaseFormat.UPPER_UNDERSCORE,
							method.buildSQLNameWithCurrentCounter());
				}
