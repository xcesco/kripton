...
public abstract class AbstractSelectCodeGenerator implements SelectCodeGenerator {
	...
	public void generate(TypeSpec.Builder classBuilder, boolean mapFields, SQLiteModelMethod method) {
		...
		generateCommonPart(method, classBuilder, methodBuilder, fieldList, true);
	...
	public void generateCommonPart(SQLiteModelMethod method, TypeSpec.Builder classBuilder, MethodSpec.Builder methodBuilder, Set<JQLProjection> fieldList, GenerationType generationType,
		...
		if (generationType.generateMethodContent) {
			SplittedSql splittedSql = SqlSelectBuilder.generateSQL(method, methodBuilder, countQuery);
			...
			if (generationType.generateCloseableCursor) {
				methodBuilder.beginControlFlow("try ($T _cursor = database().query(_sql, _sqlArgs))", cursorClass);
			} else {
				methodBuilder.addStatement("$T _cursor = database().query(_sql, _sqlArgs)", cursorClass);
			}
