...
abstract class AbstractPrimitivePrefsTransform extends AbstractPrefsTransform {
	...
	public void generateReadProperty(Builder methodBuilder, String preferenceName, TypeName beanClass, String beanName,
		...
		if (property.hasTypeAdapter()) {
			methodBuilder.addCode("$T.getAdapter($T.class).toJava(", PrefsTypeAdapterUtils.class,
		...
		if (nullable) {
			methodBuilder.addCode(
					SIMPLE_TYPE + "$L.get" + PREFS_TYPE + "($S, " + SIMPLE_TYPE + "($L==null?" + PREFS_DEFAULT_VALUE
							+ ":" + tempPre + "$L" + tempPost + "))",
					preferenceName, property.getPreferenceKey(), getter(DEFAULT_BEAN_NAME, beanClass, property),
					getter(DEFAULT_BEAN_NAME, beanClass, property));
		} else {
			methodBuilder.addCode(
					SIMPLE_TYPE + "$L.get" + PREFS_TYPE + "($S, " + SIMPLE_TYPE + tempPre + "$L" + tempPost + ")",
					preferenceName, property.getPreferenceKey(), getter(DEFAULT_BEAN_NAME, beanClass, property));
		}
